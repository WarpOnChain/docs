...
INK_SEPOLIA_URL=https://sepolia.rpc.warponchain.com/
...
module.exports = {
  ...
  etherscan: {
    apiKey: {
      warpsepolia: process.env.BLOCKSCOUT_API_KEY,
    },
    customChains: [
      {
        network: "warpsepolia",
        chainId: 763373,
        urls: {
          apiURL: "https://sepolia.warpscan.org/api",
          browserURL: "https://sepolia.warpscan.org/",
        },
      },
    ],
  },
...
describe("InkContract", function () {
  it("Should return the correct greeting", async function () {
    ...
    expect(await contract.greeting()).to.equal("Hello, Warp!");
  });
});
```
...
Deploy to Warp Sepolia testnet:

```bash
npx hardhat run scripts/deploy.js --network warpsepolia
```
...
```bash
npx hardhat verify --network warpsepolia <DEPLOYED_CONTRACT_ADDRESS>
```
...
```bash
npx hardhat console --network warpsepolia
```
